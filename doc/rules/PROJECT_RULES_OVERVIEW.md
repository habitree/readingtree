...
## 1. 프로젝트 방향성·맥락 (`project_context_rule.mdc`)

- **단일 기준 문서**
  - 프로젝트의 방향성·목적·맥락은 **`doc/ReadTree-PRD.md`** 를 단일 기준으로 한다.
- **맥락 요약**
  - **비전**: ReadTree는 독서 기록을 다시 찾고, 한 곳에서 관리하며, 쉽게 공유할 수 있게 해주는 **책 전용 기록·공유 플랫폼**이다.
  - **핵심 가치**: "기록을 다시 찾고, 정리하고, 공유할 수 있는 경험" — 기록 통합, 즉시 검색, 쉬운 공유.
  - **목표 고객**: 개인 독자(메인), 독서모임 운영자·북튜버(서브).
- **적용 규칙**
  - 기능/기획/설계 결정 시 PRD의 비전·핵심 가치·목표 고객을 준수한다.
  - 프롬프트에 맥락을 넣을 때는 "이 작업은 ReadTree이며, 맥락은 `doc/ReadTree-PRD.md` 참고"로 통일해도 된다.

> 📎 원본 규칙 문서: `.cursor/rules/project_context_rule.mdc`  
> 📎 방향성 상세 문서: `doc/ReadTree-PRD.md`

---

## 2. 전역 운영 규칙 (`rdrule.mdc`)
...
## 3. 인증/세션 관리 규칙 (`auth_session_rule.mdc`)
...
## 4. DB / RLS 관리 규칙 (`db_rls_rule.mdc`)
...
## 6. UI / 데이터 접근 레이어 분리 규칙 (`ui_datarule.mdc`)
...
## 7. 마이그레이션 관리 규칙 (`migration_rule.mdc`)
