...
이 핵심 가치는 "읽고 생각한 흔적을 다시 꺼내 쓰게 하는(사유의 축적)"을 목표로 하며, 다음 3가지 서브 가치로 분해됩니다:
...
- **필사 입력**: 
  - 텍스트 직접 입력
  - 필사 이미지 업로드 및 OCR 처리 (gogle cloud vision API 활용)
- **사진 업로드**: 
  - 책 페이지 사진 업로드
  - 여러 장 일괄 업로드 지원
...
- **기술 구현**: 
  ...
  - gogle vision API를 통한 OCR 및 텍스트 추출 (비동기 Queue 방식)
