...
/**
 * 본문 품질 공통 규칙 — 어떤 템플릿을 쓰든 섹션이 얇게 나오지 않도록 강제한다.
 * (해당 이름의 섹션이 없는 템플릿에서는 그 항목만 무시된다)
 */
export const REPORT_QUALITY_GUIDELINES = `## 본문 품질 규칙 (모든 섹션 공통 — 아래 이름의 섹션이 있을 때 각각 적용)
- 한 줄짜리 섹션 금지: 서술 섹션은 3~5문장, 목록 항목은 각각 2~3문장으로 채울 것
- "책 개요"류 섹션: 정보 정리 뒤에, 이 책이 어떤 책이고 독자가 왜 읽게 됐는지 소개하는 서술 2~3문장을 반드시 덧붙일 것
- "핵심 인사이트"류 섹션: 각 항목을 **굵은 제목** + 설명으로 쓰고, 설명에는 독자의 기록·구절에서 찾은 근거를 연결할 것
- "인상깊은 구절"류 섹션: blockquote(>)로 2~3개 인용하고, 쪽수를 아는 구절은 끝에 (p.쪽수)를 표기하며, 각 인용 아래에 이 구절이 왜 남는지 1문장 해설을 일반 문단으로 붙일 것
- "나의 생각"류 섹션: 독자의 기록 원문을 따옴표로 그대로 인용하고, 각 인용에 짧은 맥락 1문장을 이어붙일 것
- "독서 여정"류 섹션: 날짜와 기록 시점을 근거로 짧은 서사처럼 3~4문장. 진행률 퍼센트(%) 수치는 언급하지 말 것
- "종합 요약"류 섹션: 핵심 가치 2~3문장으로 정리하고, 마지막 줄은 그대로 SNS에 옮기고 싶어질 한 문장(여운 있는 질문 또는 선언)으로 끝낼 것
- 문장은 짧고 리듬 있게. 같은 종결어미의 반복과 상투적 표현("~에게 ~을 줍니다" 남발 등)을 피할 것
- 독서 노트 원문에 없는 사실·구절을 지어내지 말 것 (인용은 노트의 인용 원문만 사용)`;
...
### 1. 책 개요
- 책의 기본 정보와 독서 기간을 정리
- 이어서 이 책이 어떤 책이고 독자가 왜 읽게 됐는지 소개하는 서술 2~3문장을 반드시 포함

### 2. 핵심 인사이트
- 노트에서 추출한 3~5개의 핵심 주제를 도출
- 각 주제는 **굵은 제목** + 2~3문장 설명, 설명에는 독자의 기록·구절에서 찾은 근거를 연결

### 3. 인상깊은 구절
- 인용(quote) 노트에서 핵심 구절 2~3개를 선별
- 마크다운 인용문 블록(>) 형식 사용, 쪽수를 아는 구절은 끝에 (p.쪽수) 표기
- 각 인용 아래에 이 구절이 왜 인상적인지 1문장 해설을 일반 문단으로 덧붙임

### 4. 나의 생각 정리
- 사용자가 작성한 메모/감상의 **원문 표현과 어투를 최대한 살려서** 정리
- 원문의 핵심 문장은 그대로 인용하고, 각 인용에 짧은 맥락 1문장을 이어붙여 자연스럽게 연결
- 사용자 고유의 감정과 표현이 드러나도록 과도한 요약·재해석·의역을 지양
- 메모가 여러 개인 경우 주제별로 자연스럽게 묶되, 각 메모의 핵심 내용은 생략하지 않음

### 5. 독서 여정
- 날짜와 기록 시점을 근거로 시간순 진행 과정을 짧은 서사처럼 3~4문장으로 작성
- 독서 패턴이나 특이사항 언급, 완독한 경우 시작일~완독일 기간과 완독 성과를 강조
- 진행률 퍼센트(%) 수치는 언급하지 않음

### 6. 종합 요약
- 이 책이 독자에게 준 핵심 가치를 2~3문장으로 정리
- 마지막 줄은 그대로 공유하고 싶어질 한 문장(여운 있는 질문 또는 선언)으로 마무리
...
/** 진행률 문자열 생성 — 완독 이력이 있으면(재독 포함) 잔여 페이지 기반 %를 쓰지 않는다 */
function formatProgress(book: BookInfo): string {
  // 완독 이력(completedAt) 기준: 재독 중 current_page가 남아 있어도 "21% 진행" 같은
  // 오해를 부르는 표기를 만들지 않는다
  if (book.status === "completed" || book.completedAt) {
    const base = book.totalPages ? `${book.totalPages}쪽 완독` : "완독";
    return book.status === "rereading" ? `${base} (재독 중)` : base;
  }
